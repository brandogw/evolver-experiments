...
def test (OD_data, temp_data, vials, elapsed_time, exp_name):
    ...
    for x in vials: #main loop through each vial
        ...
        tempconfig_path =  "{0}/{1}/temp_config/vial{2}_tempconfig.txt".format(save_path,exp_name,x)
        ...
        ODset_path =  "{0}/{1}/ODset/vial{2}_ODset.txt".format(save_path,exp_name,x)
        ...
        if len(data) > 7:
            ...
            if average_OD > ODset:
                ...
                file_path =  "{0}/{1}/pump_log/vial{2}_pump_log.txt".format(save_path,exp_name,x)
                data = np.genfromtxt(file_path, delimiter=',')
                last_pump = data[len(data)-1][0]
                if ((elapsed_time - last_pump)*60) >= pump_wait:
                    MESSAGE = {'pump_binary':"{0:b}".format(control[x]), 'pump_time': time_in, 'efflux_pump_time': time_out, 'delay_interval': 0, 'times_to_repeat': 0, 'run_efflux': 1}
...
def test_chemostat (OD_data, temp_data, vials, elapsed_time, exp_name):
    ...
    for x in vials: #main loop through each vial
        ...
        if len(data) > 7: #waits for seven OD measurements (couple minutes) for sliding window
            ...
            chemoconfig_path =  "{0}/{1}/chemo_config/vial{2}_chemoconfig.txt".format(save_path,exp_name,x)
